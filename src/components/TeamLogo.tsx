...
export const TeamLogo = ({ teamId, logo, shortName = 'Team', size = 'md', className }: TeamLogoProps) => {
  ...
  return (
    <div className={cn('rounded-md bg-secondary/60 border border-white/10 overflow-hidden flex items-center justify-center', sizeClass[size], className)}>
      {resolvedLogo ? (
        <img src={resolvedLogo} alt={`${shortName} logo`} className="w-[90%] h-[90%] object-contain" loading="lazy" onError={(event) => { event.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(shortName)}&background=111827&color=ffffff&size=128`; }} />
      ) : (
